...
export const seedList = {
    refresh: function(){
        ...
        oldpapers.select('.paper-title').html(function(p){
            return(`${p.title}<a target='_blank' href='https://doi.org/${p.doi}'>${linkoutsvg}</a>`)
        })
...
eventResponse(true,'seedUpdate',seedList.refresh)
eventResponse(true,'newSeed',seedList.refresh)
eventResponse(true,'seedDeleted',seedList.refresh)
