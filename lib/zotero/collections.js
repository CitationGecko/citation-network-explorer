...
function getCollections(data, callback) {
  const url = API_ROOT + '/users/' + data.userId + '/collections?limit=1000';
  ...
  return request(opts, (error, response, body) => {
    ...
    if (parsedBody && parsedBody.error) {
      ...
      var start = 100;
      while(collections.length<(totalCollections+1)){
        
        let new_url = url + '&start=' + start;
        opts.url = new_url;
        if(start<=totalCollections){
          request(opts, (error, response, body) => {
            if (error) {
              return callback(error);
            }
            let json = JSON.parse(body)
            let new_collections = _.map(json, 'data')
            collections = collections.concat(new_collections);
          })
        }
        start += 100;
      }
...
function getContents(collectionID, data, callback) {
  ...
  return request(opts, (error, response, body) => {
    ...
    const parsedBody = JSON.parse(body);
    const dois = _.compact(_.uniq(_.map(parsedBody, 'data.DOI')));

    return callback(null, dois);
  });
}
...
function insertPaperDataIntoTemplate(template, paper, collectionId) {
  ...
  const rtn = _.merge({}, template, newData);
  return rtn;
}
 ...
 */
function addItems(papers, data, callback) {
  ...
  request(templateOpts, (templateErr, templateResponse, templateBody) => {
    const allPapersPayload = _.map(papers, (paper) => insertPaperDataIntoTemplate(templateBody, paper, data.collectionId));
